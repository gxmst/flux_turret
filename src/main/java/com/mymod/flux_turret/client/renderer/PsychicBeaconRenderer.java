...
public class PsychicBeaconRenderer implements BlockEntityRenderer<PsychicBeaconBlockEntity> {
    ...
    private static final int BASE_MODEL_DISTANCE = 64;
    ...
    public void render(PsychicBeaconBlockEntity be, float partialTick, PoseStack poseStack,
        ...
        if (Vec3.atCenterOf(be.getBlockPos()).distanceToSqr(cameraPos)
                <= BASE_MODEL_DISTANCE * BASE_MODEL_DISTANCE) {
            this.geckoRenderer.render(be, partialTick, poseStack, bufferSource, packedLight, packedOverlay);
        }

        if (state == PsychicBeaconBlockEntity.STATE_OFFLINE
        ...
        if (renderNetworkLinks && TurretClientConfig.renderBeaconNetworkLinks()) {
    ...
    private void renderNetworkLinks(PsychicBeaconBlockEntity be, int state, float partialTick,
        ...
        double renderDistance = TurretClientConfig.beaconNetworkLinkDistance();
        if (renderDistance <= 0.0) {
            return;
        }
        Vec3 cameraPos = Minecraft.getInstance().gameRenderer.getMainCamera().getPosition();
        if (Vec3.atCenterOf(be.getBlockPos()).distanceToSqr(cameraPos) > renderDistance * renderDistance) {
            return;
        }
    ...
    public boolean shouldRenderOffScreen(PsychicBeaconBlockEntity be) {
        int state = be.getBeaconState();
        return TurretClientConfig.optionalEffectsEnabled()
                && state != PsychicBeaconBlockEntity.STATE_OFFLINE
                && state != PsychicBeaconBlockEntity.STATE_FAILED;
    }
    ...
    public boolean shouldRender(PsychicBeaconBlockEntity be, Vec3 cameraPos) {
        ...
        int state = be.getBeaconState();
        boolean longRangeEffects = TurretClientConfig.optionalEffectsEnabled()
                && state != PsychicBeaconBlockEntity.STATE_OFFLINE
                && state != PsychicBeaconBlockEntity.STATE_FAILED;
        if (!longRangeEffects) {
            return center.closerThan(cameraPos, BASE_MODEL_DISTANCE);
        }
        return center.multiply(1.0D, 0.0D, 1.0D)
                .closerThan(cameraPos.multiply(1.0D, 0.0D, 1.0D), this.getViewDistance());
    }

    @Override
    public int getViewDistance() {
        return Math.max(192, (int) Math.ceil(TurretClientConfig.beaconNetworkLinkDistance()));
    }
}
